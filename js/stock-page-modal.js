 ...
 * @param {Object} cfg 
 * @param {string} cfg.collectionName - Nama koleksi Firestore (office\nppe\nsouvenir\nsupplier\n...) 
 ...
 * @param {function} [cfg.computeTotals] - (docData) => number\nstring untuk kolom Total (opsional) 
 * @param {string}   [cfg.loginUrl='/login_main.html'] 
 * @param {Object}   [cfg.ui] 
 * @param {'lg'\n'xl'\n'full'} [cfg.ui.size='xl'] - ukuran modal 
 * @param {1\n2\n3}            [cfg.ui.columns=1] - grid kolom form 
 ...
 * @param {'all'\n'filtered'}  [cfg.exportScope='all'] - export seluruh data atau hasil filter saja 
 */ 
export function initStockPageModal(cfg) { 
      ...
      <div class="modal-dialog ${sizeClass}" style="max-width:${maxWidth}px; width:100%;"> 
        <div class="modal-content"> 
          ...
          <div class="modal-body"> 
            ...
            <!-- STEP 1: Bungkus fields dengan <form> --> 
  ...
  // STEP 2: Tambah referensi formEl
  ...
  // STEP 4: Tambah fungsi validasi form (HTML5)
  function validateForm() {
    if (!formEl) return true;

    // Trim kolom teks yang required
    for (const f of fields) {
      ...
      if (!el.value) {
        el.setCustomValidity("Kolom wajib diisi");
      }
    }

    const ok = formEl.reportValidity();

    // Bersihkan pesan kustom agar tidak menahan sesudah diperbaiki
    for (const f of fields) {
      const el = document.getElementById(`${MODAL_ID}-${f.key}`);
  ...
  // STEP 5: Validasi sebelum simpan
